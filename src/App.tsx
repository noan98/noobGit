...
export default function App() {
  ...
  // #69 機密ファイル検出: ステージ前に機密ファイルを検査し、見つかれば警告ダイアログを挟む。
  //
  // `pathsToCheck` はステージ対象パスの一覧（stage_all の場合は status から集める、
  // stage_path の場合は単一パス）。`proceedAction` は警告を無視して続行するときに呼ぶ
  // 本来のステージ処理。検出されなければ `proceedAction` を直接呼ぶ。
  //
  // この関数は将来の #81（LFS ガイド）などのステージ前チェックでも再利用できるよう
  // 独立した形で定義している。
  async function stageWithSensitiveCheck(
    pathsToCheck: string[],
    proceedAction: () => void,
  ) {
    ...
    try {
      ...
      if (warnings.length > 0) {
        setSensitiveGuard({ warnings, proceedAction });
      } else {
        proceedAction();
      }
    } catch {
